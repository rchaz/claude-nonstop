...
/**
 * Find the most recently modified session across all accounts.
 *
 * @param {Array<{name: string, configDir: string}>} accounts
 * @returns {{ account: object, cwdHash: string, sessionId: string, path: string, mtime: number } | null}
 */
export function findLatestSessionAcrossProfiles(accounts) {
  let best = null;
  ...
  for (const account of accounts) {
    ...
    for (const cwdHash of cwdHashes) {
      const cwdDir = join(projectsDir, cwdHash);
      let entries;
      try {
