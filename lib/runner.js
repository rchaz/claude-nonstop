...
export async function run(claudeArgs, selectedAccount, allAccounts, options = {}) {
  ...
  // Resolve the preferred account object (if specified); mutable so it can
  // be disabled on persistent migration failure to prevent infinite loops.
  let preferredAccount = preferredAccountName
    ...
    if (result.preferredReturn) {
      ...
      if (session) {
        ...
        if (migration.success) {
          ...
          console.error('[claude-nonstop] Disabling preferred account return');
...
function runOnce(claudeArgs, account, existingSessionId, options = {}) {
  return new Promise((resolve) => {
    ...
    child.onExit(({ exitCode }) => {
      ...
      // Remove abort listener to prevent firing on a dead PTY
